...
import { get_products_shop } from "../redux/store/reducers/productReducer";
import { FaLocationDot, FaBasketShopping } from "react-icons/fa6";
const ShopPage = () => {
  const { sellerId } = useParams();
  const dispatch = useDispatch();
  const { priceRange, parPage } = useSelector((state) => state.home);
  const { products, shop } = useSelector((state) => state.product);

  useEffect(() => {
    dispatch(get_products_shop(sellerId));
  }, [dispatch, sellerId]);
  useEffect(() => {
  ...
  }, [rating, sortPrice, pageNumber, debouncedQueryProducts, state.values]);
  ...
  return (
    <div className="w-[85%] mx-auto p-16 border border-gray-300">
      <section className="">
        <div className="w-[85%] md:w-[80%] p-16 shadow-md text-white bg-rose-500 sm:w-[90%] lg:w-[90%] h-full mx-auto ">
          <h1 className="text-3xl font-bold mb-">{shop?.shopInfo?.shopName}</h1>

          <p className="">
            <FaLocationDot className="inline mr-3" />
            <span>{shop?.shopInfo?.address}</span>
          </p>
          <p>
            <FaBasketShopping className="inline mr-3" />
            <span>Số sản phẩm : {products.length}</span>
          </p>
        </div>
      </section>
              ...
              <div className="py-2 flex flex-col gap-5">
                ...
                <div>
                  <span className="text-slate-800 font-bold text-lg">
                    {formatCurrency(state.values[0])} -
                    {formatCurrency(Math.floor(state.values[1]))}
                  </span>
                </div>
              </div>
