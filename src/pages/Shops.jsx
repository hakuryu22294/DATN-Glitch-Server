import { useState, useEffect, lazy, useCallback } from "react";
import { Link } from "react-router-dom";
import { IoIosArrowForward } from "react-icons/io";
import { Range } from "react-range";
import { AiFillStar } from "react-icons/ai";
import { CiStar } from "react-icons/ci";
import Products from "../components/products/Products";
import { BsFillGridFill } from "react-icons/bs";
import { FaThList } from "react-icons/fa";
const ShopProducts = lazy(() => import("../components/products/ShopProducts"));
import Pagination from "../components/Pagination";
import { useDispatch, useSelector } from "react-redux";
import {
  price_range_product,
  query_products,
} from "../redux/store/reducers/homeReducer";
import { formatCurrency } from "../utils/index";
import debounce from "lodash.debounce";

const Shops = () => {
  const dispatch = useDispatch();
  const {
    products,
    categories,
    priceRange,
    latest_product,
    totalProduct,
    parPage,
  } = useSelector((state) => state.home);
  useEffect(() => {
    dispatch(price_range_product());
  }, [dispatch]);

  useEffect(() => {
    setState({
      values: [priceRange.low, priceRange.high],
    });
  }, [priceRange]);

  const [filter, setFilter] = useState(true);
  const [state, setState] = useState({
    values: [priceRange.low, priceRange.high],
  });
  const [rating, setRating] = useState("");
  const [styles, setStyles] = useState("grid");
  const [pageNumber, setPageNumber] = useState(1);
  const [sortPrice, setSortPrice] = useState("");
  const [category, setCategory] = useState("");

  const queryCategory = (e, value) => {
    if (e.target.checked) {
      setCategory(value);
    } else {
      setCategory("");
    }
  };

  const debouncedQueryProducts = useCallback(
    debounce((filterOptions) => {
      dispatch(query_products(filterOptions));
    }, 300),
    [dispatch]
  );

  useEffect(() => {
    debouncedQueryProducts({
      low: state.values[0],
      high: state.values[1],
      category,
      rating,
      sortPrice,
      pageNumber,
    });
  }, [
    category,
    rating,
    sortPrice,
    pageNumber,
    debouncedQueryProducts,
    state.values,
  ]);

  const resetRating = () => {
    setRating("");
    dispatch(
      query_products({
        low: state.values[0],
        high: state.values[1],
        category,
        rating: "",
        sortPrice,
        pageNumber,
      })
    );
  };

  return (
    <div>
      <section className="py-16 relative">
        <div className="w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto flex flex-wrap">
          <div className={`md:w-4/12 w-full md:pr-6 mb-6 md:mb-0`}>
            <div className="bg-base-300 rounded-lg p-4">
              <h2 className="text-3xl font-bold mb-3 text-base-content">
                Category
              </h2>
              <div className="py-2">
                {categories?.map((c, i) => (
                  <div key={i} className="flex items-center gap-2 py-1">
                    <input
                      checked={category === c.name ? true : false}
                      onChange={(e) => queryCategory(e, c.name)}
                      type="checkbox"
                      id={c.name}
                    />
                    <label className="cursor-pointer" htmlFor={c.name}>
                      {c.name}
                    </label>
                  </div>
                ))}
              </div>

              <div className="py-2 flex flex-col gap-5">
                <h2 className="text-3xl font-bold mb-3">Price</h2>
                <Range
                  step={1000000}
                  min={priceRange.low}
                  max={priceRange.high}
                  values={state.values}
                  onChange={(values) => setState({ values })}
                  renderTrack={({ props, children }) => (
                    <div
                      {...props}
                      className="w-full h-[6px] bg-slate-200 rounded-full cursor-pointer"
                    >
                      {children}
                    </div>
                  )}
                  renderThumb={({ props }) => (
                    <div
                      className="w-[15px] h-[15px] bg-[#059473] rounded-full"
                      {...props}
                    />
                  )}
                />
                <div>
                  <span className="font-bold text-lg">
                    {formatCurrency(state.values[0])} -{" "}
                    {formatCurrency(Math.floor(state.values[1]))}
                  </span>
                </div>
              </div>

              <div className="py-3 flex flex-col gap-4">
                <h2 className="text-3xl font-bold mb-3">Rating</h2>
                <div className="flex flex-col gap-3">
                  {[5, 4, 3, 2, 1].map((r, i) => (
                    <div
                      key={i}
                      onClick={() => setRating(r)}
                      className="text-orange-500 flex items-start gap-2 text-xl cursor-pointer"
                    >
                      {Array.from({ length: 5 }).map((_, index) => (
                        <span key={index}>
                          {index < r ? <AiFillStar /> : <CiStar />}
                        </span>
                      ))}
                    </div>
                  ))}
                  <div
                    onClick={resetRating}
                    className="text-orange-500 flex items-start gap-2 text-xl cursor-pointer"
                  >
                    <span>
                      <CiStar /> Reset
                    </span>
                  </div>
                </div>
              </div>
            </div>
            <div className="py-5 sm:hidden lg:block">
              <Products title="Latest Product" products={latest_product} />
            </div>
          </div>

          {/* Product Section */}
          <div className="md:w-8/12 w-full">
            <div className="pl-0 md:pl-6">
              <div className="py-4 bg-base-300 mb-10 px-3 rounded-md flex justify-between items-start border">
                <h2 className="text-lg font-medium text-base-content">
                  ({totalProduct}) Products
                </h2>
                <div className="flex justify-center items-center gap-3">
                  <select
                    onChange={(e) => setSortPrice(e.target.value)}
                    className="p-1 border outline-0 text-base-content rounded-md font-semibold"
                  >
                    <option value="">Sort By</option>
                    <option value="low-to-high">Low to High Price</option>
                    <option value="high-to-low">High to Low Price</option>
                  </select>
                  <div className="flex gap-4">
                    <div
                      onClick={() => setStyles("grid")}
                      className={`p-2 ${styles === "grid" && "bg-slate-300"} 
                      text-slate-600 hover:bg-slate-300 cursor-pointer rounded-sm`}
                    >
                      <BsFillGridFill />
                    </div>
                    <div
                      onClick={() => setStyles("list")}
                      className={`p-2 ${styles === "list" && "bg-slate-300"} 
                      text-slate-600 hover:bg-slate-300 cursor-pointer rounded-sm`}
                    >
                      <FaThList />
                    </div>
                  </div>
                </div>
              </div>
              <div className="pb-8">
                <ShopProducts products={products} styles={styles} />
              </div>
              <div>
                {totalProduct > parPage && (
                  <Pagination
                    pageNumber={pageNumber}
                    setPageNumber={setPageNumber}
                    totalItem={totalProduct}
                    parPage={parPage}
                    showItem={Math.floor(totalProduct / parPage)}
                  />
                )}
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Shops;
