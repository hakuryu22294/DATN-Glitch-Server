  ...
  return (
    <div
      className={`w-full grid ${
        styles === "grid"
          ? "grid-cols-5 md-lg:grid-cols-2 md:grid-cols-2"
    ...
    >
      {products?.map((p, i) => (
        <div
          key={i}
          className={`flex transition-all shadow-lg border border-rose-300 p-4 duration-1000 hover:shadow-md hover:-translate-y-3 ${
            styles === "grid"
              ? "flex-col justify-start items-start"
              : "justify-start items-center md-lg:flex-col md-lg:justify-start md-lg:items-start"
          } w-full gap bg-white p-1 rounded-md`}
        >
          <div
            className={
              styles === "grid"
                ? "w-full relative group  h-[200px] md:h-[170px] xs:h-[170px] overflow-hidden"
